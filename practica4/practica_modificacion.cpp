 ...
 */
std::vector<int> generate_ca(int prn_id, int prn_id_g1, int length, bool verbose = false) {
    ...
    auto [tap3, tap4] = g2_taps[prn_id_g1];
    ...
    for (int i = 0; i < length; i++) {
        ...
        int g1_out = g1[tap3-1] ^ g1[tap4-1];
 ...
 */
int main() {
    int prn_id, length, prn_id_g1;

    std::cout << "Generador C/A de GPS" << std::endl;
    std::cout << "====================" << std::endl;
    std::cout << "ID del satelite para g2 (1-32): ";
    std::cin >> prn_id;
    std::cout << "Longitud de la secuencia de salida: ";
    std::cin >> length;

    
    std::cout << "ID del satelite para g1 (1-32)" << std::endl;
    std::cin >> prn_id_g1;
    ...
    if (prn_id_g1 < 1 || prn_id_g1 > 32) {
        std::cerr << "Error: ID de satelite debe estar entre 1 y 32." << std::endl;
        return 1;
    }

    std::cout << std::endl;
    std::vector<int> seq = generate_ca(prn_id, prn_id_g1, length, true);
